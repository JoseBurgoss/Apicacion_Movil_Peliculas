...
export default function SeriesListScreen({ navigation }) {
  ...
  const fetchSeries = async () => {
    try {
      ...
      let url = `https://api.themoviedb.org/3/discover/tv?api_key=${API_KEY}&language=es-ES&sort_by=${sortOption}${category ? `&with_genres=${category}` : ''}`;
      ...
      if (response.ok) {
        ...
        // Fetch additional details for each series
        const detailedSeriesData = await Promise.all(
          seriesData.map(async (series) => {
            const detailsUrl = `https://api.themoviedb.org/3/tv/${series.id}?api_key=${API_KEY}&language=es-ES&append_to_response=credits`;
  ...
  const searchSeries = async (query) => {
    if (query.length > 0) {
      try {
        const API_KEY = 'd8353dcaa6b589573747e5ada48834e6';
        const url = `https://api.themoviedb.org/3/search/tv?api_key=${API_KEY}&language=es-ES&query=${query}&page=1&include_adult=false`;
        ...
        if (response.ok) {
          ...
          setSearchResults(detailedSeriesData);
        } else {
          console.log('Error al obtener datos:', response.status);
        }
      } catch (error) {
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.title2}>Series Populares</Text>
